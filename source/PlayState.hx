	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			if(!ClientPrefs.lowQuality) {
				...
				vCutsce = new BGSprite('old/smallCutscene', -240, -140, 0.0, 0.0, ['video']);
			...
			switch (daSong)
			{
				...
				if (isStoryMode){
					...
					new FlxTimer().start(5.8, function(tmr:FlxTimer)
						...
						new FlxTimer().start(5.82, function(tmr:FlxTimer)
							{
	...
	function fixArrowsPos(babyArrow:StrumNote, player:Int, ?middleScrollFunni:Bool = false)
		{
			...
			// debug moment
			#if debug
			trace(babyArrow.ID + ": X = " + babyArrow.x + " - " + ": Y = " + babyArrow.y);
			#end
